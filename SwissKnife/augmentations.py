    ...
    if level == 0:
        ...
        augmentation = iaa.Sequential(
            [
                ...
                sometimes(
                    iaa.CoarseDropout(p=0.05, size_percent=0.05, per_channel=False)
                ),
            ],
            random_order=True,
        )
        ...
        augmentation = iaa.Sequential(
            [
                ...
                sometimes(
                    iaa.CoarseDropout(p=0.08, size_percent=0.1, per_channel=False)
                ),
        ...
        augmentation = iaa.Sequential(
            [
                ...
                sometimes(
                    iaa.CoarseDropout(p=0.15, size_percent=0.1, per_channel=False)
                ),
                sometimes(
                    iaa.CoarseDropout(p=0.15, size_percent=0.3, per_channel=False)
                ),
                sometimes(
                    iaa.CoarseDropout(p=0.15, size_percent=0.9, per_channel=False)
                ),
                sometimes(iaa.GaussianBlur(sigma=(0, 1.2))),
            ],
            random_order=True,
        )
    else:
        raise ValueError(f"Unsupported augmentation level: {level}. Choose from 0, 1, 2, or 3.")
